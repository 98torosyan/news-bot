 ...
 *
 *   Filled-count is genuinely ordinal. ●●○ is visibly more than ●○○ without
 *   anyone having to be told which colour outranks which, and it stays true in
 *   greyscale, in both themes, and for a reader who cannot see colour at all.
 *
 *   And it keeps red in reserve. If every routine Fed meeting is red, nothing
 *   is left for the day something actually breaks.
 *
 * U+25CF and U+25CB are ordinary text characters, not emoji: no colour font is
 * involved, no variation selector is needed, and they inherit the message's own
 * text colour on every client. That is why they cannot render differently on
 * one person's phone than on another's — which is exactly what 🗓 and 🏛 do.
 *
 * WHAT THIS IS NOT
 *
